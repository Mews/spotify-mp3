...
def test_get_tracks_from_youtube_search(mock_videos_search):
    mock_videos_search.return_value.result.return_value = load_data_from_file(
        "youtube_search_result.txt"
    )

    tracks = get_tracks_from_youtube_search(
        "search query", limit=5, download_cover=False
    )
