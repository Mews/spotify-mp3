...
from mutagen.id3 import ID3NoHeaderError, APIC, ID3, TIT2, TPE1, TALB, TOAL
...
from src.spotifymp3.gui.utils import ObservableList, get_root_tk, DownloadOptions
...
class App(tk.Tk):
    def __init__(self, *args, **kwargs):
        ...
        self.SPOTIFY_CLIENT = create_spotify_client(get_config("spotify", "client_id"), get_config("spotify", "client_secret"))
    ...
    def process_convert_track(self, convert_track:ConvertTrack, download_options:DownloadOptions, retries:int=3):
        ...
        if convert_track.youtube_track:
            ...
            match_results = convert_spotify_track_to_youtube(convert_track.spotify_track, search_count=download_options.youtube_search_limit, download_cover=download_options.download_covers)
        ...
        if download_options.save_metadata and convert_track.spotify_track:
            ...
            audio["TIT2"] = TIT2(encoding=3, text=convert_track.spotify_track.name)  # Title
            audio["TPE1"] = TPE1(encoding=3, text=", ".join(convert_track.spotify_track.artists))  # Artists
            audio["TALB"] = TALB(encoding=3, text=convert_track.spotify_track.album) # Album
            audio["TOAL"] = TOAL(encoding=3, text=convert_track.spotify_track.album) # Album

            # Add album cover
            if convert_track.spotify_track.cover:
                with BytesIO() as img_byte_array:
                    convert_track.spotify_track.cover.save(img_byte_array, format="JPEG")  # Save as JPEG
    ...
    def start_processing_queue(self, donwload_options:DownloadOptions):
        ...
        for track in self.queue:
            ...
            future = self.executor.submit(self.process_convert_track, track, donwload_options)
...
class QueueViewer(tk.Frame):
    def __init__(self, master, *args, **kwargs):
        ...
        self.sheet = Sheet(self, headers=["Source", "Title", "Artists", "Status"], auto_resize_columns=75)
    ...
    def update_sheet_data(self):
        ...
        for track in queue:
            if track.spotify_track:
                new_sheet_data.append(["Spotify", track.spotify_track.name, ", ".join(track.spotify_track.artists), track.status_message()])
            elif track.youtube_track:
                new_sheet_data.append(["Youtube", track.youtube_track.name, track.youtube_track.artist, track.status_message()])
