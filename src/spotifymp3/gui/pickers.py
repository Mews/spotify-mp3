...
from src.spotifymp3 import youtube, spotify
from src.spotifymp3.settings import change_config, get_config, reset_default_configs
...
class PlaylistPreview(tk.Frame):
    def __init__(self, master, *args, **kwargs):
        ...
        self.song_count_label = tk.Label(self, text="0 songs", font=("TkDefaultFont", 13))
    ...
    def update_cover(self):
        ...
        self.cover = self.cover.resize((target_width, round(target_width/cover_aspect_ratio)))
...
class TrackPreview(tk.Frame):
    def __init__(self, master, *args, **kwargs):
        ...
        self.artists_label = tk.Label(self, text="Artists: ", font=("TkDefaultFont", 13))
    ...
    def update_preview(self):
        ...
        self.length_label.config(text="Track length: "+f"{int(minutes)}:{round(seconds):02d}")
...
class YoutubePlaylistPicker(tk.Toplevel):
    def __init__(self, master, handler, *args, **kwargs):
        ...
        self.download_covers_checkbox = tk.Checkbutton(self.input_frame, text="Download track covers?", variable=self.download_covers_var)
        self.download_covers_checkbox.pack(expand=True)

        self.add_button = tk.Button(self.input_frame, text="Add playlist", command=self.on_add_button_press)
    ...
    def on_entry_change(self, var, index, mode):
        ...
        if url:
            ...
            Thread(target=self.load_playlist_in_thread, args=(url,), daemon=True).start()
        else:
    ...
    def on_add_button_press(self):
        ...
        try:
            tracks = youtube.get_playlist_tracks(self.link_var.get(), download_covers=self.download_covers_var.get())
        except Exception as e:
...
class YoutubeTrackPicker(tk.Toplevel):
    def __init__(self, master, handler, *args, **kwargs):
        ...
        self.download_cover_checkbox = tk.Checkbutton(self.input_frame, text="Download track cover?", variable=self.download_cover_var)
        self.download_cover_checkbox.pack(expand=True)

        self.add_button = tk.Button(self.input_frame, text="Add video", command=self.on_add_button_press)
        self.add_button.pack(anchor="n", expand=True)
    ...
    def on_add_button_press(self):
        ...
        try:
            track = youtube.get_track_from_url(self.link_var.get(), download_cover=self.download_cover_var.get())
        except Exception as e:
...
class SpotifyPlaylistPicker(tk.Toplevel):
    ...
    def on_add_button_press(self):
        ...
        try:
            tracks = spotify.get_playlist_tracks(self.SPOTIFY_CLIENT, self.link_var.get(), download_covers=self.download_covers_var.get())
        except Exception as e:
            print("Error getting tracks from playlist", self.link_var.get())
            print(traceback.format_exc())
...
class SpotifyTrackPicker(tk.Toplevel):
    def __init__(self, master, handler, *args, **kwargs):
        ...
        self.add_button = tk.Button(self.input_frame, text="Add song", command=self.on_add_button_press)
        self.add_button.pack(anchor="n", expand=True)
    ...
    def on_add_button_press(self):
        ...
        try:
            track = spotify.get_track_from_url(self.SPOTIFY_CLIENT, self.link_var.get(), download_cover=self.download_cover_var.get())
        except Exception as e:
            print("Error getting track", self.link_var.get())
            print(traceback.format_exc())
...
class DownloadOptionsPicker(tk.Toplevel):
    def __init__(self, master=None, handler=None, *args, **kwargs):
        ...
        output_folder_entry = tk.Entry(self, textvariable=self.output_folder_var, state='readonly')
        output_folder_entry.pack(pady=(0, 5), padx=10, fill='x')
        
        browse_button = tk.Button(self, text="Browse...", command=self.open_folder_picker)
        browse_button.pack(pady=(0, 10))

        tk.Label(self, text="Youtube matches limit:").pack()
        youtube_limit_spinbox = tk.Spinbox(self, from_=1, to=1000, textvariable=self.youtube_limit_var)
        youtube_limit_spinbox.pack(pady=(0, 10))

        match_cover_checkbox = tk.Checkbutton(self, text="Match cover to thumbnails?", variable=self.match_cover_var)
        match_cover_checkbox.pack(pady=(5, 10))

        add_metadata_checkbox = tk.Checkbutton(self, text="Add metadata to songs?", variable=self.add_metadata_var)
        add_metadata_checkbox.pack(pady=(5, 10))

        download_button = tk.Button(self, text="Download queue", command=self.download_queue)
    ...
    def show_warning(self, message):
        ...
        tk.Button(warning_window, text="OK", command=warning_window.destroy).pack(pady=(0, 10))
...
class SettingsEditor(tk.Toplevel):
    def __init__(self, master=None, *args, **kwargs):
        ...
        self.client_secret_var = tk.StringVar(value=get_config("spotify", "client_secret"))
        ...
        link = tk.Label(self, text="How do I get my client id and client secret?", fg="blue", cursor="hand2")
        link.pack(pady=10)
        ...
        self.reset_button = tk.Button(self, text="Reset Default Settings", command=self.on_reset)
    ...
    def has_unsaved_changes(self):
        return (
            self.client_id_var.get() != get_config("spotify", "client_id") or
            self.client_secret_var.get() != get_config("spotify", "client_secret")
        )
    ...
    def on_reset(self):
        response = messagebox.askyesno("Reset Settings", "Are you sure you want to restore the default settings?")
        if response:
