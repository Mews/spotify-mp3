...
class TopBar(tk.Frame):
    def __init__(self, master, *args, **kwargs):
        ...
        self.youtube_playlist_button = tk.Button(
            self,
            image=self.youtube_playlist_icon,
            text="Youtube playlist",
            compound=tk.TOP,
            font=("", 12),
            padx=10,
            pady=5,
            command=self.open_youtube_playlist_picker,
        )
        self.youtube_playlist_button.pack(side=tk.LEFT, pady=5, padx=(5, 0), anchor="w")

        self.youtube_track_button = tk.Button(
            self,
            image=self.youtube_plus_icon,
            text="Youtube video",
            compound=tk.TOP,
            font=("", 12),
            padx=10,
            pady=5,
            command=self.open_youtube_track_picker,
        )
        self.youtube_track_button.pack(side=tk.LEFT, pady=5, padx=(0, 5), anchor="w")

        self.spotify_playlist_button = tk.Button(
            self,
            image=self.spotify_playlist_icon,
            text="Spotify playlist",
            compound=tk.TOP,
            font=("", 12),
            padx=10,
            pady=5,
            command=self.open_spotify_playlist_picker,
        )
        self.spotify_playlist_button.pack(side=tk.LEFT, pady=5, anchor="w")

        self.spotify_track_button = tk.Button(
            self,
            image=self.spotify_plus_icon,
            text="Spotify track",
            compound=tk.TOP,
            font=("", 12),
            padx=10,
            pady=5,
            command=self.open_spotify_track_picker,
        )
